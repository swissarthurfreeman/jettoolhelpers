...
int main (int argc, char* argv[]) {
    xAOD::TEvent event;     // Prepare to read the input file(s)
    xAOD::TStore store;     // xAOD::TEvent represents a series of events, e.g. sets of jets.

    
    if (argc != 3) {        // Check the arguments 
        printf("USAGE: %s <num events, -1 for all> <input file>\n",argv[0]);    // input file contains all the .root events.
        exit(1);
    }
    ...
    std::unique_ptr<IInputBase> myH1D = std::make_unique<HistoInput>(MakeHistoInput("myH1D", histFile, histName1D, "pt", "float", true));
    std::unique_ptr<IInputBase> myH2D = std::make_unique<HistoInput>(MakeHistoInput("myH2D", histFile, histName2D, "pt", "float", true, "abseta", "float", true));

    if(myH1D->initialize() != 1 || myH2D->initialize() != 1)
        exit(1);

    Long64_t numEvents = atoll(argv[1]); // Parse the arguments, first argument is number of events to read from provided root file.
    ...
    printf("Processing %lld events...\n", numEvents);         // Check the number of events, if number is larger than the available ones, cap...

    for (Long64_t iEntry = 0; iEntry < numEvents; ++iEntry) { // Loop over the events
        event.getEntry(iEntry);                               // Get the event
        if (iEntry%100 == 0)
        ...
        JetContext jc; // Make an empty JetContext object
        ...
        store.clear();  // End of the event, clear the store to free memory
    }
    return 0;
}
#endif
